...
def edit_member(member_id):
    ...
    db = get_db()
    member_cur = db.execute('update members set name = ?, email = ?, level = ? where id = ?', [name, email, level, member_id])
    db.commit()

    updated_member_cur = db.execute('select id, name, email, level from members where id = ?', [member_id])
    updated_member = updated_member_cur.fetchone()

    return jsonify({'updated_member': {'id': updated_member['id'], 'name': updated_member['name'], 'email': updated_member['email'], 'level': updated_member['level']}})
